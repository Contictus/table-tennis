...
export default function MatchPage() {
  ...
  return <main className="game-page"><header className="game-header"><div className="brand compact"><span className="brand-mark">●</span><div><strong>RALLY</strong><small>TABLE TENNIS</small></div></div><div className="scoreboard"><div><small>YOU</small><strong className="score-you">{String(homeScore).padStart(2, '0')}</strong></div><span>/</span><div><small>OPPONENT</small><strong>{String(awayScore).padStart(2, '0')}</strong></div></div><div className="game-actions"><div className="connection-pill"><span className={`status-dot ${connectionClass}`} />{connectionLabel}</div><IconButton label="Toggle sound" onClick={() => setSoundOn(!soundOn)}>{soundOn ? <Volume2 size={18} /> : <span>×</span>}</IconButton><IconButton label="Help" onClick={() => setHelpOpen(true)}><CircleHelp size={18} /></IconButton><IconButton label="Pause menu" onClick={() => setPauseOpen(!pauseOpen)}><Pause size={18} /></IconButton></div></header><div className="game-stage"><div className="stage-label" role="status" aria-live="polite">{pointLabel ?? phaseLabel}</div>{pointLabel && <div className="point-callout" role="status">{pointLabel}</div>}<GameCanvas soundOn={soundOn} /><aside className="rally-card"><small>CURRENT RALLY</small><strong>{String(rally).padStart(2, '0')}</strong><span>FIRST TO 11</span></aside></div>{helpOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setHelpOpen(false) }}><MatchHelp onClose={() => setHelpOpen(false)} /></div>}{pauseOpen && <div className="modal-backdrop"><section className="pause-modal"><div className="eyebrow">MATCH PAUSED</div><h2>Take a breath.<br /><em>Take a shot.</em></h2><p>The match continues while this menu is open.</p><Button onClick={() => setPauseOpen(false)}>Continue</Button><Button variant="ghost" onClick={() => navigate('/')}>Leave match</Button></section></div>}<footer className="game-footer"><span>YOU VS PLAYER</span><span>ROOM {roomCode}</span><span>DRAG TO MOVE</span></footer></main>
}
